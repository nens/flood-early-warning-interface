...
function IssuedWarningsTable() {
  ...
  return (
    <div style={{padding: '1rem'}}>
      <select onChange={(e) => {setFilter(e.target.value); setPage(0);}}>
        <option value={''}>-- Filter on alarm name --</option>
        {alarms.map(alarm => (
          <option key={alarm.uuid} value={alarm.uuid}>{alarm.name}</option>
        ))}
      </select>
      <div className={styles.WarningsTable}>
        <div className={styles.tr}>
          <div
            ...
            onClick={() => clickOrdering('triggeredat')}
          >
            Triggered at {isOrdering('triggeredat')}
          </div>
          <div
            className={styles.thtd}
            onClick={() => clickOrdering('name')}
          >
            Name {isOrdering('name')}
          </div>
          <div
            className={styles.thtd}
            onClick={() => clickOrdering('level')}
          >
            Trigger level {isOrdering('level')}
          </div>
          <div
            className={styles.thtd}
            onClick={() => clickOrdering('value')}
          >
