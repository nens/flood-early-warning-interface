...
function SettingsModal(props: SettingsModalProps) {
  ...
  const [dashboardUrl, setDashboardUrl] = useState<string>(
    trainingDashboards && trainingDashboards[0] ? trainingDashboards[0].url : ""
  );

  const { isTraining, setCurrentConfigSlug, resetConfigSlug } = useContext(ConfigContext);
  const { now, isDateEdited, setEditedNow, resetEditedNow } = useContext(TimeContext);
    ...
    <Modal close={closeFunction} title="Settings">
      {!isDateEdited ? (
        <>
          ...
          >
            {isTraining ? <option value="">Return to live dashboard</option> : null}
        ...
        <>
          ...
          <p>
            Time set to {now.toLocaleString()}.
