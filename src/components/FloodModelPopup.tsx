...
function FloodModelPopup({ raster, time }: FloodModelPopupProps) {
  ...
  const rasterIntersections: RasterIntersection[] = clickLocation
    ? [
        {
          uuid: raster,
  ...
  const rasterEventsResponse = useRasterEvents(rasterIntersections, now, end);
  ...
  if (fetched) {
    ...
    const closestIndex = arrayMin(events, (event) =>
      Math.abs(time.getTime() - event.timestamp.getTime())
    );
    ...
    <Popup position={clickLocation}>
      {fetched ? (
        value!.value !== null ? (
          ...
          "Water depth: no water"
        )
      ) : (
        "Fetching water depth..."
      )}
    </Popup>
  );
}
