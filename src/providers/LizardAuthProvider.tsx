...
export const LizardAuthContext = React.createContext<{ bootstrap: Bootstrap | null }>({
  bootstrap: null,
});
...
function LizardAuthProvider({ children }: WithChildren) {
  ...
  if (bootstrapResponse.status === "success") {
    ...
    return (
      <LizardAuthContext.Provider value={{ bootstrap }}>{children}</LizardAuthContext.Provider>
    );
  } else if (bootstrapResponse.status === "loading") {
    return <p>Waiting for authentication...</p>;
  } else {
    return <p>Error when loading authentication: {JSON.stringify(bootstrapResponse)}.</p>;
  }
}
